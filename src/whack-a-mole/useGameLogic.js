...
export const DEFAULT_GAME_DATA = [
...

  {
    ...
    answers: [
      ...
      },
      {
        ...
        text: "Grapes",
        image:
          "https://static.vecteezy.com/system/resources/thumbnails/007/697/457/small_2x/a-mouth-watering-isometric-icon-of-grapes-vector.jpg",
      },
    ],
  ...
  },
  {
    ...
    promptImage:
      "https://static.vecteezy.com/system/resources/previews/043/231/957/non_2x/simple-square-shape-icon-vector.jpg",
    displayMode: "text_only",
    answers: [
  ...
  },
  {
    ...
    promptImage:
      "https://img.freepik.com/free-vector/hand-drawn-clown-fish-cartoon-illustration_23-2150683251.jpg?w=360",
    displayMode: "text_only",
    answers: [
      { id: "fish", text: "Fish" },
      { id: "frog", text: "Frog" },
...
const makeEmptyMoles = (n) =>
  Array.from({ length: n }, () => ({ up: false, content: null }));
...
export default function useGameLogic(
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      currentPopupTimeouts.forEach((timeoutId) =>
  ...
  const endGameInternal = useCallback(() => {
    ...
    if (finalSoundRef.current) {
      ...
      finalSoundRef.current
        .play()
        .catch((e) => console.error("Error playing final sound:", e));
    }
  ...
  const nextRound = useCallback(() => {
    ...
    roundTimerRef.current = window.setTimeout(() => {
      ...
      const roundData = buildRoundOptions(
        data,
        moleCount,
      ...
      setCurrentProgress({
        current: currentQuestionIndexRef.current + 1,
        total: data.length,
      });
    }, roundDelayMs);
  ...
  const showFeedback = useCallback((text, color) => {
    ...
    window.setTimeout(
      () => setFeedback({ show: false, text: "", color: "", hammerHit: false }),
      1000
    );
  }, []);

  const createPointPopup = useCallback(
    (points, moleElement, containerElement) => {
  ...
  const handleMoleHit = useCallback(
    (e, moleRefs, containerRef) => {
      ...
      if (ansId == null) {
        ...
        window.setTimeout(
          () => setFeedback((f) => ({ ...f, hammerHit: false })),
          150
        );
        showFeedback("❌", "red");
        roundLockedRef.current = false;
